...
- [x] 骨架屏替换"加载中"文字：`globals.css` 加 shimmer 动画，`components/dashboard/panel-skeleton.tsx` 面板级骨架（大屏整页 + 传感器卡片网格两套），仅首载无数据时显示
- [x] 数据表虚拟滚动：`@tanstack/react-virtual` 替换原分页方案（分页在 SSE 2s 全量推送下会跳动；固定 35px 行高规避祖先 scale 对测量的干扰，表头吸顶，一屏到底）
- [x] ~~其余图表顺势迁 ECharts~~（P2 已完成全部迁移，recharts 已移除，无遗留）
- [ ] Uptime Kuma 监控三件事：6041 探活、大屏 HTTP 200、磁盘使用率，异常推企业微信/钉钉（**服务端操作，提示词已交付**）
- [ ] taosdump 定时全量备份（**服务端操作，提示词已交付**）

**P3 完成记录**：ts-check 通过；浏览器实测两页面骨架屏正常出现并被替换、数据表滚动到底正常（87 条，表头吸顶）、无控制台错误。
